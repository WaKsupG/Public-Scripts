...
local window = create:Win(7304627461)

local tap1 = window:addtap("Reaper 2")

local page1 = tap1:addpage()

page1:Ti("Autofarming")
...
page1:Toggle("Autofarm",nil, function(value)
        ...
        for i,v in pairs(game:GetService("Workspace").Living:GetChildren()) do
            if game.Players.LocalPlayer.Character and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") then
                if v.Name == _G.Mob and v.Humanoid.Health > 0 then
                    repeat wait() 
                        if not game:GetService("Workspace").Food:FindFirstChildWhichIsA("Part") or not game:GetService("Workspace").Food:FindFirstChildWhichIsA("MeshPart") then
...
page1:Toggle("Auto Quest",nil, function(value)
    _G.AutoQuest = value
...
local drop = page1:DropDown("Select","Mob",Mob, function(value)
    _G.Mob = value
end)

local drop = page1:DropDown("Select","Quest",Quest, function(value)
    _G.Quest = value
end);
...
local getslider = page2:Slder("Distance",0,10,0, function(value)
    _G.DIS = value
end);

page2:Toggle("Auto Eat",nil, function(value)
    _G.Eat = value
...
page2:Toggle("Auto Equip",nil, function(value)
    _G.Equip = value
